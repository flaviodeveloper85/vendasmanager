...
class Vendas_vendedor_model extends CI_Model{
	...
	// retorna todos os processos de vendas negociando/vendido/cancelado q pertence ao vendedor logado
	function getSell($idUser)
	{	

		$date = date('m');
		return $this->db->query("SELECT t1.name_produto, t1.qtde, t2.*, t3.name from tb_produtos t1 inner join tb_sell t2 on t1.id_produto = t2.id_produto inner join tb_user t3 on t2.id_user = t3.id_user where MONTH(t2.created_sell) = '$date' and t2.id_user = '$idUser' order by t2.id_sell desc")->result();
